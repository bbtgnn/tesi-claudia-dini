...
export interface IRenderer extends IDrawContext {
	...
	loadImage(url: string, options?: { maxHeight?: number }): Promise<IDrawableImage>;
	...
	/**
	 * Save the current canvas as an image (e.g. PNG). Optional; only supported by
	 * renderers that have a native save API (e.g. P5Renderer uses p5 saveCanvas).
	 */
