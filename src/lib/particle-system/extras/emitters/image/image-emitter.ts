import type { Simulation } from '$lib/particle-system/core/simulation';
import type { Context, Emitter, ParticleDescriptor, Vec2 } from '$lib/particle-system/core/types';
...
export class ImageEmitter implements Emitter {
	...
	async init(renderer: IRenderer): Promise<void> {
		const img = await renderer.loadImage(this.config.imageFile, {
			maxHeight: this.config.imageMaxHeight ?? settings.imageMaxHeight
		});
		...
		const workingImage =
			this.scale > 1 ? renderer.createResizedCopy(img, Math.floor(img.width / this.scale), 0) : img;
		...
		for (const scaledPolygon of scaledPolygons) {
			const pixels = Image.getPixelsInPolygon(internalImage, scaledPolygon, scaledBoundaryDistance);
		...
		this.frontiers = this.config.frontiers.map((fn) => fn(img.width, img.height));
		this.imageRef = img;
	}
